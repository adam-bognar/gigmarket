...
public class UpdateGigCommandHandlerTests
{
    [Fact]
    public async Task Handle_Should_Call_GigService_UpdateGigAsync()
    {
        var gigService = Substitute.For<IGigService>();
        var handler = new UpdateGigCommandHandler(gigService);

        var gigId = Guid.NewGuid();
        var request = CreateValidRequest();
        var command = new UpdateGigCommand(gigId, request);

        var expectedGig = CreateGigDto(gigId);

        gigService
            .UpdateGigAsync(gigId, request, Arg.Any<CancellationToken>())
            .Returns(expectedGig);
    ...
    private static UpdateGigRequest CreateValidRequest()
    {
        return new UpdateGigRequest(
            ...
            Requirements:
            [
                new UpdateGigRequirementRequest(
            ...
            AdditionalPhotoUrls:
            [
                "additional-photo-1.jpg"
            ],
            VideoUrl: null,
            Status: GigStatus.Active);
    }

    private static GigDto CreateGigDto(Guid gigId)
    {
        return new GigDto(
            Id: gigId,
